...
do
    ...
    monitor_cpu_usage "picoquic" "${output_folder}server_usage_${i}.csv" $SERVER_PID & 
    MONITOR_SERVER_PID=$!
    echo "Started monitoring server CPU usage."

    # Start the client
    ./picoquicdemo_client -n test -o ./client_files/. 0.0.0.0 4433 "$file_request" &
    CLIENT_PID=$!
    echo "Client started with PID: $CLIENT_PID"


    # # Start monitoring client CPU usage
    monitor_cpu_usage "picoquicdemo_cl" "${output_folder}client_usage_${i}.csv" $CLIENT_PID & 
    MONITOR_CLIENT_PID=$!
